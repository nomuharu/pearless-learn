...
| `pipeline.py:feature_engineering` | 16特徴量計算の実装仕様 | preserve | 16 特徴量の順序・名称・API マッピングを ADR-0002 で確定。ta ライブラリを使用し先頭 NaN 行をドロップする | `16特徴量の順序と名称: [MA60乖離率, 天井距離率(ceiling_distance), MA20乖離率, MA10乖離率, 前足比, HLO比率(hlo_ratio), diff_HLO_and_Average, CCI(20), RSI(9), 振れ幅比率(swing_ratio), VWAP乖離率, BB%B(BB(20)), MACDヒストグラム, ATR比率(atr_ratio), 時間帯sin, 時間帯cos]; 正準定義は models/configs.py の ALL_FEATURES; 価格水準(円)単位の特徴量は分布シフトを起こすためすべて比率化（ADR-0002 改訂 2026-06-10）; ta ライブラリ使用; 先頭NaN行をドロップ; dayofweekは削除済み` |
